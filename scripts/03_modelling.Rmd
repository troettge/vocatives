...
When a consonant is present, is it typically an obstruent or a sonorant?
...
v_acc_voc_cons %>%
  group_by(case_broad) %>%
  summarise(prop=mean(obstr_pres))
...
# output of this model is very strange!
mod_obstr <- brm(obstr_pres ~ case_broad_sum + 
                      (1 + case_broad_sum | family1) + 
                      (1 + case_broad_sum | macroarea),
            data=v_acc_voc_cons, family="bernoulli", cores=2,
            control=list(adapt_delta=0.99),
            prior=priors_consonants)
# mod_vl_obstr <- readRDS("../models/mod_vl_obstr.rds")
saveRDS(mod_obstr, "../models/mod_vl_obstr.rds")
summary(mod_obstr)
pp_check(mod_obstr)
marginal_effects(mod_obstr)
stanplot(mod_obstr, type="dens")
```

Manual estimates.

```{r}
mod_obstr_psamp <- as.matrix(posterior_samples(mod_obstr)[,1:2])
# accusative-like - all probabilities
mod_obstr_acc_probs <- plogis(mod_obstr_psamp %*% matrix(c(1, min(v_acc_voc_cons$case_broad_sum))))
round(mean(mod_obstr_acc_probs), 5)
round(quantile(mod_obstr_acc_probs, c(0.025, 0.975)), 5)

# vocative-like - all probabilities
mod_obstr_voc_probs <- plogis(mod_obstr_psamp %*% matrix(c(1, max(v_acc_voc_cons$case_broad_sum))))
round(mean(mod_obstr_voc_probs), 5)
round(quantile(mod_obstr_voc_probs, c(0.025, 0.975)), 5)

# difference
round(mean(mod_obstr_voc_probs - mod_obstr_acc_probs), 5)
round(quantile(mod_obstr_voc_probs - mod_obstr_acc_probs, c(0.025, 0.975)), 5)
```
