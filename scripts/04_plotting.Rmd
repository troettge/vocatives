...
# aggregate raw data over language
v_aggr <- v %>%
...
# same, but only non-zero suffixes
v_aggr_non_zero <- v %>%
    filter(case_broad != "nom-like" & !is.na(form_add_s)) %>% 
    dplyr::select(language, case_broad, prosody_clean,
         vowel_pres, cons_pres, cons_end, vowel_end, 
         obstr_vl_pres, v_long, family1, latitude, longitude) %>% 
  group_by(language, case_broad, family1) %>%
...
# aggregate raw data over language for suffixes only
v_aggr_suffix <- v %>%
    filter(case_broad != "nom-like") %>% 
...
# aggregated over families for suffixes only
...
The plots below are for suffixes only!
...
map <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  ...
  geom_point(data = v_aggr, aes(x = longitude, y = latitude, fill = cons_pres, group = NA), size = 4, shape = 21, color = "black", stroke = 0.3,
...
These plots (included in the paper) are for all morphemes, not just suffixes.
...
map_combo <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill = "lightgray", colour = "white", size = 0.2) +
  geom_point(data = v_aggr_non_zero, aes(x = longitude, y = latitude, fill = cons_pres, group = NA), size = 4, shape = 21, color = "black", stroke = 0.3,
             alpha = 0.8, position = position_jitter(0.5,0.5)) +
   coord_map(xlim = c(-180, 180),ylim = c(-60, 80)) +
  #ylim(-60,80) +
  #coord_fixed(ylim = c(-60, 80), ratio = 1) +
