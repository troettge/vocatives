...
prosody_post$case_broad <- c("acc-like", "vocative")
...
ggplot(data = prosody_post, aes(x = case_broad, y = mean)) +
  ...
  labs(title = "Results: Prosodic modulation",
...
cons_pres_post$case_broad <- c("acc-like", "vocative")
...
cons_pres_suff_post$case_broad <- c("acc-like", "vocative")    
...
# load in model output for overall consonant model
cons_vl_model <- readRDS('../models/mod_c_final.rds')
...
cons_final_post$case_broad <- c("acc-like", "vocative")
...
cons_vl_post$case_broad <- c("acc-like", "vocative")
...
vow_pres_post$case_broad <- c("acc-like", "vocative")
...
vow_quant_post$case_broad <- c("acc-like", "vocative")
...
vow_qual_post$case_broad <- rep(c("acc-like", "vocative"), 3)
...
ggplot(data = vow_qual_post, aes(x = case_broad, y = mean, fill = case_broad)) +
  ...
  labs(title = "Results: Vowel quality",
       subtitle = "Vocatives have more mid vowels and less high vowels\n",
...
map <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  ...
  geom_point(data = v_aggr, aes(x = longitude, y = latitude, color = cons_pres, group = NA, size = 4),
...
# save  
ggsave("../plots/maps_consonants.pdf", width = 10, height = 3.6)
ggsave("../plots/maps_consonants.png", width = 10, height = 3.6)
...
map_combo <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill = "lightgray", colour = "white", size = 0.2) +
  geom_point(data = v_aggr_suffix, aes(x = longitude, y = latitude, color = cons_pres, group = NA), alpha = 0.8, size = 3, position = position_jitter(0.1)) +
   coord_map(xlim = c(-180, 180),ylim = c(-60, 80)) +
  #ylim(-60,80) +
  #coord_fixed(ylim = c(-60, 80), ratio = 1) +
  facet_grid(~case_broad) +
  scale_colour_gradient(limits = c(0,1), breaks = c(0,0.25,0.5,0.75,1), low = "#4dac26", high = "#d01c8b") +
  labs(title = "A - Map: Presence of consonants in suffixes",
       subtitle = "Vocatives often have no consonants\n") +
  theme_voc +
  theme(
    legend.position = "right",
